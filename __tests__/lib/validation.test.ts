...
describe('validateContactForm', () => {
  ...
  it('should validate message length (max 2000 characters)', () => {
    ...
    expect(result.isValid).toBe(false);
    expect(result.errors.message).toBe('Message must be 2000 characters or less');
  });
  ...
  it('should validate invalid LinkedIn URL', () => {
    ...
    expect(result.isValid).toBe(false);
    expect(result.errors.linkedin).toBe('Please enter a valid LinkedIn profile URL (e.g., https://linkedin.com/in/yourname)');
  });
